...
public class Pose_J : MonoBehaviour {
    ...
    //成功したポーズの判定で使う
    public string posename = "pose_J";
    //ポーズが決まったか
    public bool DecidePose_J;
    ...
    {
        //ポーズガイドの画像
        pose_J = gameObject.GetComponent<Image>();
        r = pose_J.GetComponent<Image>().color.r;
        g = pose_J.GetComponent<Image>().color.g;
        b = pose_J.GetComponent<Image>().color.b;
        alpha = pose_J.GetComponent<Image>().color.a;
        //プレイヤーの関節の角度など
    ...
    {
        //ポーズの画像の情報
        pose_J.GetComponent<Image>().color = new Color(r, g, b, alpha);
        //画像をプレイヤーの上、X、Yの調整
    ...
    //ポーズの画像を表示させる
    public void JPoseDisplaytrue()
    {
        alpha = 1.0f;
    }
    //ポーズの画像を表示させない
    public void JPoseDisplayfalse()
    {
        alpha = 0.0f;
    }
}
