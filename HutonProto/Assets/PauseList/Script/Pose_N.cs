...
public class Pose_N: MonoBehaviour {
    ...
    //成功したポーズの判定で使う
    public string posename = "pose_N";
    //ポーズが決まったか
    public bool DecidePose_N;
    ...
    {
        //ポーズガイドの画像
        pose_N = gameObject.GetComponent<Image>();
        r = pose_N.GetComponent<Image>().color.r;
        g = pose_N.GetComponent<Image>().color.g;
        b = pose_N.GetComponent<Image>().color.b;
        alpha = pose_N.GetComponent<Image>().color.a;
        //プレイヤーの関節の角度など
    ...
    //ポーズの画像を表示させる
    public void NPoseDisplaytrue()
    {
        alpha = 1.0f;
    }
    //ポーズの画像を表示させない
    public void NPoseDisplayfalse()
    {
        alpha = 0.0f;
    }
}
