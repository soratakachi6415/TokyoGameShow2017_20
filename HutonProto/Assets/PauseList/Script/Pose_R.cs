...
public class Pose_R : MonoBehaviour {
    PlayerStatus playerstatus;

    /*ポーズ_バナナの判定を行う*/
    //「バナナ」ポーズの画像を所得
    protected Image pose_R;
    ...
    //成功したポーズの判定で使う
    public string posename = "pose_R";

    void Start()
    {
        //ポーズガイドの画像
        pose_R = gameObject.GetComponent<Image>();
        r = pose_R.GetComponent<Image>().color.r;
        g = pose_R.GetComponent<Image>().color.g;
        b = pose_R.GetComponent<Image>().color.b;
        alpha = pose_R.GetComponent<Image>().color.a;
        //プレイヤーの関節の角度など
    ...
    void Update()
    {
        ...
        /***************************************/
        pose_R.GetComponent<Image>().color = new Color(r, g, b, alpha);
    ...
    //ポーズの画像を表示させる
    public void RPoseDisplaytrue()
    {
        alpha = 1.0f;
    }

    //ポーズの画像を表示させない
    public void RPoseDisplayfalse()
    {
        alpha = 0.0f;
    }
}
