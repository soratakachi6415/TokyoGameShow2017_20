...
public class Pose_Race : MonoBehaviour {

    PlayerStatus playerstatus;

    /*ポーズ_バナナの判定を行う*/
    //「バナナ」ポーズの画像を所得
    protected Image pose_Racecs;
    ...
    //成功したポーズの判定で使う
    public string posename = "pose_Racecs";

    void Start()
    {
        //ポーズガイドの画像
        pose_Racecs = gameObject.GetComponent<Image>();
        r = pose_Racecs.GetComponent<Image>().color.r;
        g = pose_Racecs.GetComponent<Image>().color.g;
    ...
    //ポーズの画像を表示させる
    public void RacecsPoseDisplaytrue()
    {
        alpha = 1.0f;
    }

    //ポーズの画像を表示させない
    public void RacecsPoseDisplayfalse()
    {
        alpha = 0.0f;
    }
}
