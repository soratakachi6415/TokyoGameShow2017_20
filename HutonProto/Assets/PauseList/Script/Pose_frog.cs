...
public class Pose_frog : MonoBehaviour {
    ...
    //成功したポーズの判定で使う
    public string posename = "pose_frog";
    //ポーズが決まったか
    public bool DecidePose_frog;
    ...
    {
        //ポーズガイドの画像
        pose_frog = gameObject.GetComponent<Image>();
        r = pose_frog.GetComponent<Image>().color.r;
        g = pose_frog.GetComponent<Image>().color.g;
        b = pose_frog.GetComponent<Image>().color.b;
        alpha = pose_frog.GetComponent<Image>().color.a;
        //プレイヤーの関節の角度など
    ...
    {
        //ポーズの画像の情報
        pose_frog.GetComponent<Image>().color = new Color(r, g, b, alpha);
        //画像をプレイヤーの上、X、Yの調整
    ...
    //ポーズの画像を表示させる
    public void frogPoseDisplaytrue()
    {
        alpha = 1.0f;
    }
    //ポーズの画像を表示させない
    public void frogPoseDisplayfalse()
    {
        alpha = 0.0f;
    }
}
