...
public class Pose_painfullPose2 : MonoBehaviour {
    //0612 変更完了
    ...
    /*ポーズ_大の字の判定を行う*/
    //「大」ポーズの画像を所得
    private Image pose_painfullpose2;
    ...
    //成功したポーズの判定で使う
    public string posename = "pose_painfullpose2";
    //ポーズが決まったか
    public bool DecidePose_painfullpose2;
    ...
    {
        //ポーズガイドの画像
        pose_painfullpose2 = gameObject.GetComponent<Image>();
        r = pose_painfullpose2.GetComponent<Image>().color.r;
        g = pose_painfullpose2.GetComponent<Image>().color.g;
        b = pose_painfullpose2.GetComponent<Image>().color.b;
        alpha = pose_painfullpose2.GetComponent<Image>().color.a;
        //プレイヤーの関節の角度など
        playerstatus = GameObject.FindGameObjectWithTag("PlayerStatus").GetComponent<PlayerStatus>();
        anglePM = playerstatus.anglePM;
        painfullpose2PoseDisplayfalse();
    }
    ...
    //ポーズの画像を表示させる
    public void painfullpose2PoseDisplaytrue()
    {
        alpha = 1.0f;
    }
    //ポーズの画像を表示させない
    public void painfullpose2PoseDisplayfalse()
    {
        alpha = 0.0f;
    }
}
