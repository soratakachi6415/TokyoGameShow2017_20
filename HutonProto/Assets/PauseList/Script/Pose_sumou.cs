...
public class Pose_sumou : MonoBehaviour {
    //0612 変更完了
    ...
    //成功したポーズの判定で使う
    public string posename = "pose_sumou";
    //ポーズが決まったか
    public bool DecidePose_sumou;
    ...
    {
        //ポーズガイドの画像
        pose_sumou = gameObject.GetComponent<Image>();
        r = pose_sumou.GetComponent<Image>().color.r;
        g = pose_sumou.GetComponent<Image>().color.g;
        b = pose_sumou.GetComponent<Image>().color.b;
        alpha = pose_sumou.GetComponent<Image>().color.a;
        //プレイヤーの関節の角度など
    ...
    {
        //ポーズの画像の情報
        pose_sumou.GetComponent<Image>().color = new Color(r, g, b, alpha);
        //画像をプレイヤーの上、X、Yの調整
    ...
    //ポーズの画像を表示させる
    public void sumouPoseDisplaytrue()
    {
        alpha = 1.0f;
    }
    //ポーズの画像を表示させない
    public void sumouPoseDisplayfalse()
    {
        alpha = 0.0f;
    }
}
