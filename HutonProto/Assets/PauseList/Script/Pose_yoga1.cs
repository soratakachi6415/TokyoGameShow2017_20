...
public class Pose_yoga1 : MonoBehaviour {

    PlayerStatus playerstatus;

    /*ポーズ_バナナの判定を行う*/
    //「バナナ」ポーズの画像を所得
    protected Image pose_yoga1;
    ...
    //成功したポーズの判定で使う
    public string posename = "pose_yoga1";

    void Start()
    {
        //ポーズガイドの画像
        pose_yoga1 = gameObject.GetComponent<Image>();
        r = pose_yoga1.GetComponent<Image>().color.r;
        g = pose_yoga1.GetComponent<Image>().color.g;
        b = pose_yoga1.GetComponent<Image>().color.b;
        alpha = pose_yoga1.GetComponent<Image>().color.a;
        //プレイヤーの関節の角度など
        playerstatus = GameObject.FindGameObjectWithTag("PlayerStatus").GetComponent<PlayerStatus>();
        anglePM = playerstatus.anglePM;
        yoga1PoseDisplayfalse();
    }
    ...
    void Update()
    {
        ...
        /***************************************/
        pose_yoga1.GetComponent<Image>().color = new Color(r, g, b, alpha);
    ...
    //ポーズの画像を表示させる
    public void yoga1PoseDisplaytrue()
    {
        alpha = 1.0f;
    }

    //ポーズの画像を表示させない
    public void yoga1PoseDisplayfalse()
    {
        alpha = 0.0f;
    }
}
