...
public class State_BodyBuli : MonoBehaviour {
    ...
    /****ポーズオブジェクトの取得****/
    // ボディービルのオブジェクトを取得
    private Image _Bodybuli;
    public Pose_BodyBuli _bodybuli;

    private bool _chance = false;
    ...
    void Start () {
        ...
        _Bodybuli = GameObject.Find("Pose_BodyBuli").GetComponent<Image>();
        _bodybuli = _Bodybuli.GetComponent<Pose_BodyBuli>();
    ...
    void Update()
    {
        if (_bodybuli.L_arm_flag == true ||
            _bodybuli.R_arm_flag == true ||
            _bodybuli.L_leg_flag == true ||
            _bodybuli.R_leg_flag == true)
        {
            _chance = true;
            ChanceDisPlayTrue();
        }
    ...
    {
        if ((_bodybuli.R_arm_flag == true &&
            _bodybuli.L_arm_flag == true) ||
            (_bodybuli.R_leg_flag == true &&
            _bodybuli.L_leg_flag == true))
        {
            _posemanager._Pose = PoseManager.PoseState.Bodybuilding;
        }
