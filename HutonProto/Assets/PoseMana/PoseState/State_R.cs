...
public class State_R : MonoBehaviour {
    ...
    /****ポーズオブジェクトの取得****/
    // 開脚のオブジェクトを取得
    private Image _AlphaR;
    public Pose_R _alphaR;
    ...
    void Start()
    {
        ...
        _AlphaR = GameObject.Find("Pose_R").GetComponent<Image>();
        _alphaR = _AlphaR.GetComponent<Pose_R>();
    ...
    void Update()
    {
        if (_alphaR.L_arm_flag == true ||
            _alphaR.R_arm_flag == true ||
            _alphaR.L_leg_flag == true ||
            _alphaR.R_leg_flag == true)
        {
            _chance = true;
            ChanceDisPlayTrue();
        }
    ...
    {
        if ((_alphaR.R_arm_flag == true &&
            _alphaR.L_arm_flag == true) ||
            (_alphaR.R_leg_flag == true &&
            _alphaR.L_leg_flag == true))
        {
            _posemanager._Pose = PoseManager.PoseState.AlphaR;
        }
