...
public class State_Sumou : MonoBehaviour {

    public PoseManager _posemanager;

    /****ポーズオブジェクトの取得****/
    // 開脚のオブジェクトを取得
    private Image _Sumou;
    public Pose_sumou _sumou;
    ...
    void Update()
    {
        if (_sumou.L_arm_flag == true ||
            _sumou.R_arm_flag == true ||
            _sumou.L_leg_flag == true ||
            _sumou.R_leg_flag == true)
        {
            _chance = true;
            ChanceDisPlayTrue();
        }
    ...
    {
        if ((_sumou.R_arm_flag == true &&
            _sumou.L_arm_flag == true) ||
            (_sumou.R_leg_flag == true &&
            _sumou.L_leg_flag == true))
        {
